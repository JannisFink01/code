...
Die \textit{Turn Relevancy} bewertet, ob die Antworten des Assistenten im jeweiligen Gesprächskontext relevant sind. Hierfür werden Assistant-Turns gemeinsam mit vorausgehenden Gesprächsschritten betrachtet und danach beurteilt, ob die jeweilige Antwort zum bisherigen Dialog passt \cite{DeepEvalTurnRelevancy}. Die \textit{Role Adherence} untersucht dagegen, ob der Assistent über den Dialog hinweg der in \texttt{chatbot\_role} festgelegten Rolle entspricht \cite{DeepEvalRoleAdherence}. Mit \textit{Knowledge Retention} wird bewertet, ob im Gespräch eingeführte Fakten in späteren Turns beibehalten werden, während \textit{Conversation Completeness} untersucht, ob die im Gespräch auftretenden Nutzerbedürfnisse über den gesamten Verlauf adressiert werden \cite{DeepEvalKnowledgeRetention,DeepEvalConversationCompleteness}.

Für dialogbasierte RAG-Systeme stellt DeepEval darüber hinaus Multi-Turn-Varianten von RAG-Metriken bereit. \textit{Turn Faithfulness} bewertet, ob die Aussagen des Assistenten durch den für den jeweiligen Turn abgerufenen Retrieval-Kontext gestützt werden \cite{DeepEvalTurnFaithfulness}. \textit{Turn Contextual Relevancy} untersucht, welcher Anteil des bereitgestellten Retrieval-Kontexts für die jeweilige Nutzereingabe relevant ist \cite{DeepEvalTurnContextualRelevancy}. Davon zu unterscheiden ist \textit{Turn Contextual Precision}, das berücksichtigt, ob relevante Inhalte innerhalb der abgerufenen Ergebnisse vor weniger relevanten Inhalten eingeordnet werden \cite{DeepEvalTurnContextualPrecision}. \textit{Turn Contextual Recall} bewertet schließlich, ob der Retrieval-Kontext die Informationen enthält, die zur Erreichung des erwarteten Ergebnisses benötigt werden \cite{DeepEvalTurnContextualRecall}.
