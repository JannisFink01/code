...
Neben frei definierbaren Kriterien stellt DeepEval vordefinierte Metriken für wiederkehrende Qualitätsdimensionen von Multi-Turn-Systemen bereit. Diese arbeiten auf einem \texttt{ConversationalTestCase} und berücksichtigen den Gesprächskontext bei der Bewertung \cite{DeepEvalMultiTurnMetrics}.

Die \textit{Turn Relevancy} bewertet, ob die Antworten des Assistenten im jeweiligen Gesprächskontext relevant sind. Hierfür werden Assistant-Turns gemeinsam mit vorausgehenden Gesprächsschritten betrachtet und danach beurteilt, ob die jeweilige Antwort zum bisherigen Dialog passt \cite{DeepEvalTurnRelevancy}. Die \textit{Role Adherence} untersucht dagegen, ob der Assistent über den Dialog hinweg der in \texttt{chatbot\_role} festgelegten Rolle entspricht \cite{DeepEvalRoleAdherence}. Mit \textit{Knowledge Retention} wird bewertet, ob im Gespräch eingeführte Informationen in späteren Turns berücksichtigt und nicht widersprüchlich wiedergegeben werden. Die \textit{Conversation Completeness} untersucht, ob die im Gespräch auftretenden Nutzerbedürfnisse über den gesamten Verlauf adressiert werden \cite{DeepEvalKnowledgeRetention,DeepEvalConversationCompleteness}.

Eine weitere vordefinierte Multi-Turn-Metrik ist die \textit{Goal Accuracy}. Sie bewertet, in welchem Maß das System die aus den Nutzeräußerungen abgeleiteten Ziele erreicht. Hierzu werden die vom System gewählten Schritte sowie deren Eignung zur Erreichung des jeweiligen Ziels betrachtet \cite{DeepEvalGoalAccuracy}. Die Metrik berücksichtigt damit sowohl die Zielerreichung als auch die Qualität und Einhaltung des dafür verfolgten Vorgehens.

Für dialogbasierte RAG-Systeme stellt DeepEval darüber hinaus Multi-Turn-Metriken bereit, welche die Verwendung des abgerufenen Kontexts bewerten. \textit{Turn Faithfulness} untersucht, ob die Aussagen des Assistenten durch den für den jeweiligen Turn bereitgestellten Retrieval-Kontext gestützt werden \cite{DeepEvalTurnFaithfulness}. \textit{Turn Contextual Relevancy} bewertet dagegen, in welchem Umfang der abgerufene Kontext für die jeweilige Nutzereingabe relevant ist \cite{DeepEvalTurnContextualRelevancy}. Während \textit{Turn Faithfulness} damit die Verwendung des bereitgestellten Wissens bei der Antwortgenerierung betrachtet, richtet sich \textit{Turn Contextual Relevancy} auf die Qualität der zuvor abgerufenen Inhalte.

Die vordefinierten Metriken und Conversational G-Eval erfüllen damit unterschiedliche Funktionen. Während die vordefinierten Metriken festgelegte Qualitätsdimensionen wie Relevanz, Rollentreue oder Retrieval-Qualität abbilden, ermöglicht Conversational G-Eval die Definition anwendungsspezifischer Bewertungskriterien. Welche dieser Metriken für die Untersuchung des Systemprompts und der RAG-Pipeline eingesetzt werden, wird im weiteren Verlauf der Arbeit methodisch begründet.
