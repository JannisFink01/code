...
def run_evaluation(prompt_file: str, version: str):
    ...
    async def prompt_callback(
        ...
        resp = tutor_llm.chat.completions.create(
    ...
    print(f"\nStarte Evaluation ({len(test_cases)} Testfälle, sequenziell)...")
    ...
    for i, tc in enumerate(test_cases):
        ...
        print(
            f"  [{i+1}/{len(test_cases)}] {meta.get('topic','?')[:40]} | {meta.get('behavior','?')[:30]}"
        )
        try:
            result = evaluate(test_cases=[tc], metrics=metrics)
    ...
    print(f"\n  Ergebnis: {len(test_cases) - failed}/{len(test_cases)} erfolgreich")
